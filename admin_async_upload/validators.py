...
class StorageFileValidator(object):
    ...
    messages = {
        ...
        "extension": _(
            u"Extension {extension} not allowed. Allowed extensions are: {allowed_extensions}"
        ),
        "min_size": _(
            u"File {name} too small ({size} bytes). The minimum file size is {min_size} bytes."
        ),
        "max_size": _(
            u"File {name} too large ({size} bytes). The maximum file size is {max_size} bytes."
        ),
    }
